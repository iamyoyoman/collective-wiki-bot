...
def get_card_extra(stats):
    ...
    i_cost, i_hp, i_atk, i_tribal_type, i_creator_name, i_artist_name, i_realm = None, None, None, None, None, None, None
    ...
    extras = [i_object_type, i_affinity, i_cost, i_hp, i_atk, i_tribal_type, i_creator_name, i_artist_name, i_realm]
    stats += extras
...
def update_infobox(stats):
    ...
    info_box = info_template.format(i_name.replace("?", "-").replace("!", "-"), i_name,
                                    i_affinity.replace("None", "Neutral"), i_rarity, i_tribal_type, i_realm,
...
def get_leaderboard():
    ...
    leaderboard_list = get(url_leaderboard).json()['multi']

    qs_edit['title'] = "Top Decks " + datetime.datetime.now().strftime("%b-%Y")

    for deck in leaderboard_list:

        i_user = deck['username']
        print(i_user)
        i_deck = deck['cards']
        i_hero = deck['hero_name']

        decklist = deck_template.format(i_hero, i_user, deck['deck_rank'] )

        for name, number in i_deck.items():
            decklist = decklist + deck_template_repeat.format(number, name)

        pl_edit['text'] = pl_edit['text'] + "{|" + decklist + "|}\n" \

    pl_edit['text'] = pl_edit['text'] + "[[" + "Category:Decks" + "]]" + "[[" + "Category:Top Decks" + "]]"
